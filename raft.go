...
func GetTheLastIndex(r *raft.Raft) uint64 {
...
func (hs httpServer) addHandler(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
	...
	index := fmt.Sprint(GetTheLastIndex(hs.r))
	// set payload to raft
	b := setPayload{}
	err = json.Unmarshal(bs, &b)
	if err != nil {
		log.Printf("Could not parse payload: %s", err)
		http.Error(w, http.StatusText(http.StatusBadRequest), http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		log.Printf("Could not encode key-value in http request: %s", err)
		http.Error(w, http.StatusText(http.StatusBadRequest), http.StatusBadRequest)
		return
	}
...
func (hs httpServer) defaultHandler(w http.ResponseWriter, r *http.Request) {
	index := int(GetTheLastIndex(hs.r))
...
func (hs httpServer) delHandler(w http.ResponseWriter, r *http.Request) {
	key := r.URL.Query().Get("key")

	if _, ok := hs.db.Load(key); !ok {
...
func main() {
	...
	http.HandleFunc("/add", hs.addHandler)
	http.HandleFunc("/get", hs.getHandler)
	http.HandleFunc("/getall", hs.getallHandler)
	http.HandleFunc("/join", hs.joinHandler)
	http.HandleFunc("/del", hs.delHandler)
